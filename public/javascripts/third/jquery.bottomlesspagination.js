	...
	$.bottomlessPagination = function(callerSettings){
		settings=$.extend({
			...
			if($('div.pagination').size()>0){
				...
				$('.pagination_links').append("<div class='live_pagination'>"+"<a class='more_links' style='cursor:pointer;'>Mostrar mais "+settings.objName+"...</a>"+"</div>");
			}
			...
			$.fn.addrows=function(data){
				...
				if(data.length===0){
					...
					$('.pagination_links').append("<div class='no_pagination'>"+"Não existem mais "+settings.objName+"..."+"</div>");
					return false;
				}
